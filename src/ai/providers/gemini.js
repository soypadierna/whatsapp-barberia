...
async function interpretarConfirmacion(texto, datosActuales) {
          ...
          parameters: {
            ...
            properties: {
              accion: {
                ...
                description: 'confirmar si acepta (ej. "sí", "todo bien", "correcto", "dale", "ok", "está todo correcto"); cambiar si pide modificar un dato puntual; cancelar si quiere abandonar; no_claro si no se entiende',
              },
              campo: { type: 'string', description: 'Si accion es "cambiar", qué campo: servicio, barbero, fecha, u hora. Vacío si no aplica.' },
              valorNuevo: { type: 'string', description: 'Si accion es "cambiar", el nuevo valor mencionado. Vacío si no aplica.' },
            },
            required: ['accion'],
          },
        },
  ...
  const model = genAI.getGenerativeModel({
    ...
    tools: toolsConfirmacion,
    toolConfig: { functionCallingConfig: { mode: 'ANY' } }, // FUERZA la llamada a función, no permite respuesta en texto libre
  });
...
Determina si confirma la cita tal cual está, si quiere cambiar algo puntual, o si cancela. Sé generoso interpretando afirmaciones informales en español (ej. "todo bien", "esta correcto", "ok", "dale así", "perfecto", "confirmo" cuentan como confirmar).`;
  ...
  logger.mensaje(`[interpretarConfirmacion/gemini] texto="${texto}" → call=${JSON.stringify(call?.args || null)}`);
